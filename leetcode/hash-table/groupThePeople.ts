...
function groupThePeople(groupSizes: number[]): number[][] {
  ...
  for (let person = 0; person < groupSizes.length; person++) {
    const group = groupSizes[person];
    if (!groupSizesMap.get(group)) groupSizesMap.set(group, []);
    groupSizesMap.set(group, [...groupSizesMap.get(group), person]);
    if (groupSizesMap.get(group).length === group) {
      result.push(groupSizesMap.get(group));
      groupSizesMap.set(group, []);
    }
  }
  return result
}
